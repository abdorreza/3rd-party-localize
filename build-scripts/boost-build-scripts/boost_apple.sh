...
: ${OSXBUILDDIR:=`pwd`/macosx/build}
: ${PREFIXDIR:=`pwd`/ios/prefix}
: ${IOSFRAMEWORKDIR:=`pwd`/ios/framework}
: ${OSXFRAMEWORKDIR:=`pwd`/macosx/framework}
...
cleanEverythingReadyToStart()
{
    ...
    rm -rf iphone-build iphonesim-build macosx-build
    rm -rf $IOSBUILDDIR
    rm -rf $OSXBUILDDIR
    rm -rf $PREFIXDIR
...
buildBoostForIPhoneOS()
{
    ...
    ./b2 -j16 --build-dir=macosx-build --stagedir=macosx-build/stage toolset=clang cxxflags="-std=c++11 -stdlib=libc++ -arch i386 -arch x86_64" linkflags="-stdlib=libc++" link=static threading=multi macosx-version=${OSX_SDKVERSION} stage
    doneSection
}
...
scrunchAllLibsTogetherInOneLibPerPlatform()
{
    ...
    for NAME in $BOOST_LIBS; do
        ...
        $OSX_DEV_CMD lipo "macosx-build/stage/lib/libboost_$NAME.a" -thin i386 -o $OSXBUILDDIR/i386/libboost_$NAME.a
        $OSX_DEV_CMD lipo "macosx-build/stage/lib/libboost_$NAME.a" -thin x86_64 -o $OSXBUILDDIR/x86_64/libboost_$NAME.a
    done
